...
export function adjustHousePositionsRelativeToPlatform84x72() {
  return new Promise((resolve) => {
  ...
  for (let i = 0; i < houseList.length; i++) {
        for (let j = 0; j < platformList84x72.length; j++) {
          ...
          if (houseList[i].overlapsWith(platformList84x72[j])) {
            ...
            houseList[i].placeHouseAwayFromPlatform84x72(platformList84x72[j]);
          }
...
export async function ajustAllHouses(house,randomPlatform) {
    ...
    await adjustHousePositionsRelativeToPlatform84x72();
 };
