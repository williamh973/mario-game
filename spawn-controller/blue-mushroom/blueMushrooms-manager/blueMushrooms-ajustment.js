...
export async function checkIfBlueMushroomAreAdjusted() {
  for (let i = 0; i < 1; i++) {
    await adjustBlueMushroomsPositionsRelativeToRedMushroom(
      blueMushroomList,
      redMushroomList,
    );
  }
...
function adjustBlueMushroomsPositionsRelativeToRedMushroom(
  blueMushroomList,
  redMushroomList,
) {
    ...
    for (let i = 0; i < blueMushroomList.length; i++) {
      for (let j = 0; j < redMushroomList.length; j++) {
        if (blueMushroomList[i].overlapsWith(redMushroomList[j])) {
          ...
          blueMushroomList[i].placeBlueMushroomAwayFromRedMushroom(
            redMushroomList[j],
          );
        }
      }
