export function createBoombaSpriteRight() {
  const goomba = new Image();
  goomba.src = "../../assets/images/goombaSpriteRight.png";
  return goomba;
}
export let goombaSpriteRight = createBoombaSpriteRight(
  "../../assets/images/goombaSpriteRight.png",
);

export function createBoombaSpriteLeft() {
  const goomba = new Image();
  goomba.src = "../../assets/images/goombaSpriteLeft.png";
  return goomba;
}
export let goombaSpriteLeft = createBoombaSpriteLeft(
  "../../assets/images/goombaSpriteLeft.png",
);
