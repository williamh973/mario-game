...
import {
  createBoombaSpriteLeft,
  createBoombaSpriteRight,
  goombaSpriteLeft,
  goombaSpriteRight,
} from "./gombas-assets.js";
...
export class Goomba {
  ...
  placeGoombasRandomPlatformList500x43(referencePlatform) {
    ...
    let randomNumberX = Math.floor(
      randomFactorX * (referencePlatform.width - this.width / 1.2),
    );
  ...
  moving() {
    if (this.isMovingRight) {
      if (
        ...
        this.onPlatform.position.x + this.onPlatform.width
      ) {
        this.image = this.sprites.right;
